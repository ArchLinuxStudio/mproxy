...
void rewrite_header() {
  ...
  char *p0 = strchr(p, '\0');
  char *p5 =
      strstr(header_buffer, "HTTP/"); /* "HTTP/" 是协议标识 如 "HTTP/1.1" */
  int len = strlen(header_buffer);
  if (p) {
    char *p1 = strchr(p + 7, '/');
    if (p1 && (p5 > p1)) {
...
// not used
void stop_server() { kill(m_pid, SIGKILL); }
...
const char *get_work_mode() {
  ...
  if (strlen(remote_host) == 0) {
    if (io_flag == FLG_NONE) {
      ...
      //在远端终点服务器运行的,带简单混淆的http代理服务器
      return "start as remote forward proxy and do decode data when recevie "
             "data";
    }
  } else {
    if (io_flag == FLG_NONE) {
